...
- (void)keyboardWillShow:(NSNotification *)n {
    ...
    frame.size.height = screenHeight - NAVIGATION_BAR_HEIGHT - keyboardHeight - toolbar.frame.size.height;
    NSLog(@"%f - %d - %f - %f = %f",screenHeight, NAVIGATION_BAR_HEIGHT, keyboardHeight, toolbar.frame.size.height, frame.size.height);
    NSLog(@"keyboardWillShow %@ %@ %@ %@",
            NSStringFromCGRect([UIScreen mainScreen].applicationFrame),
            NSStringFromCGRect(self.navigationController.toolbar.frame),
            NSStringFromCGRect(keyboardRect),
    ...
    toolbarFrame.origin.y = tv.frame.origin.y + tv.frame.size.height;
...
- (void)resizeTableView:(BOOL)animate {
    ...
    timeline.hidden = NO;
//    tv.frame = CGRectMake(0, 44, width, height - 554);
//    toolbar.frame = CGRectMake(0, height - 600, width, 46);
    CGFloat timelineHeight = [UIScreen mainScreen].applicationFrame.size.height - NAVIGATION_BAR_HEIGHT - tv.frame.size.height - toolbar.frame.size.height;
    timeline.frame = CGRectMake(0, toolbar.frame.origin.y + toolbar.frame.size.height, width, timelineHeight);
    if (animate) {
        [UIView commitAnimations];
    }
}
...
- (void)requestFailed:(NSString *)i withError:(NSError *)error {
    ...
    UIAlertView *a = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"post_error_title", @"") message:NSLocalizedString(@"post_error_msg", @"") delegate:self
                                      cancelButtonTitle:@"OK"
                                      otherButtonTitles:nil];
    [a show];
    [a release];
